...
module.exports = {
    run: async (message, args, bot) => {
        ...
        if (message.member.roles.cache.get('823665644989710416') || message.member.roles.cache.get('838813818904903730')) {
            ...
            await member.send(`You have been banned from the server for the reason: ${reason}`).catch(() => console.log('Can\'t send a MP')).catch(() => console.log('Can\'t send a MP'))
            await bot.channels.cache.get(config.logsModChannel).send(`<:important:823909697857912923> ${member.user.tag} has been banned by ${message.author} for the reason: ${reason}`)
        } else {
